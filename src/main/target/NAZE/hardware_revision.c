...
uint8_t detectSpiDevice(void)
{
    ...
    ENABLE_SPI_CS;
    spiTransfer(NAZE_SPI_DEVICE, in, out, sizeof(out));
    DISABLE_SPI_CS;
    ...
    delay(50);
    ENABLE_SPI_CS;
    spiTransferByte(NAZE_SPI_DEVICE, MPU_RA_WHO_AM_I | MPU6500_BIT_RESET);
    in[0] = spiTransferByte(NAZE_SPI_DEVICE, 0xff);
    DISABLE_SPI_CS;
